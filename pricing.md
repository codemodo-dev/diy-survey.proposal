# Pricing Proposal: DIY Survey & Panel Platform

*v1.0.0 | 27 Februari 2026*

---

## Tim Pengerjaan

Tim terdiri dari 3 role dengan kapasitas masing-masing 4 jam/hari (part-time).

| Role | Per Bulan |
|------|-----------|
| Backend Developer | Rp 10.000.000 |
| Frontend Developer | Rp 10.000.000 |
| UI/UX Designer | Rp 6.000.000 |
| **Total/Bulan** | **Rp 26.000.000** |

---

## Tipe 1 — Beli Putus

Harga tetap per phase dengan 3 role (Backend, Frontend, Designer). Perubahan scope di luar SOW dikenakan biaya tambahan. Acceptance period 14 hari kerja setelah setiap delivery — jika tidak ada feedback tertulis dalam periode tersebut, dianggap approved dan termin berikutnya wajib dibayarkan.

**Catatan durasi:** Kolom durasi menampilkan estimasi minimum–maksimum. Harga yang tercantum dihitung berdasarkan durasi minimum. Jika pengerjaan mencapai upper bound, **harga tetap sama dan tidak berubah** — ini adalah keunggulan model beli putus.

| Phase | Estimasi Durasi | Dasar Perhitungan | Backend | Frontend | Designer | Total |
|-------|----------------|-------------------|---------|----------|----------|-------|
| Phase 1 — Core System | 2 bln | 2 bln | Rp 20 jt | Rp 20 jt | Rp 12 jt | **Rp 52.000.000** |
| Phase 2 — Survey Designer | 6–7 bln | 6 bln | Rp 60 jt | Rp 60 jt | Rp 36 jt | **Rp 156.000.000** |
| Phase 3 — Survey Portal | 3–4 bln | 3 bln | Rp 30 jt | Rp 30 jt | Rp 18 jt | **Rp 78.000.000** |
| Phase 4 — Analytics & Disbursement | 2–3 bln | 2 bln | Rp 20 jt | Rp 20 jt | Rp 12 jt | **Rp 52.000.000** |
| Phase 5 — Fraud Detection | 2 bln | 2 bln | Rp 20 jt | Rp 20 jt | Rp 12 jt | **Rp 52.000.000** |
| **Total MVP — Phase 1–5** | **15–18 bln** | **15 bln** | Rp 150 jt | Rp 150 jt | Rp 90 jt | **Rp 390.000.000** |

### Struktur Pembayaran — MVP (Phase 1–5)

| Termin | % | Nominal | Timing |
|--------|---|---------|--------|
| DP | 30% | Rp 117.000.000 | Kontrak ditandatangani, sebelum mulai kerja |
| Termin 2 | 30% | Rp 117.000.000 | Phase 2 selesai & diterima client |
| Termin 3 | 30% | Rp 117.000.000 | Phase 4 selesai & diterima client |
| Termin 4 | 10% | Rp 39.000.000 | MVP delivered & acceptance sign-off |
| **Total** | **100%** | **Rp 390.000.000** | |

### Post-MVP (Opsional, setelah MVP stabil)

Post-MVP dikerjakan setelah MVP berjalan stabil dan diterima client. Urutan prioritas fitur ditentukan bersama sebelum pengerjaan dimulai. Acceptance period 14 hari kerja berlaku sama seperti MVP.

| Phase | Estimasi Durasi | Dasar Perhitungan | Backend | Frontend | Designer | Total |
|-------|----------------|-------------------|---------|----------|----------|-------|
| Post-MVP | ~2 bln | 2 bln | Rp 20 jt | Rp 20 jt | Rp 12 jt | **Rp 52.000.000** |

### Struktur Pembayaran — Post-MVP

| Termin | % | Nominal | Timing |
|--------|---|---------|--------|
| DP | 50% | Rp 26.000.000 | Kontrak Post-MVP ditandatangani, sebelum mulai kerja |
| Termin 2 | 50% | Rp 26.000.000 | Post-MVP delivered & acceptance sign-off |
| **Total** | **100%** | **Rp 52.000.000** | |

---

## Tipe 2 — Retainer / Part-Time Bulanan

Bayar per bulan dengan 3 role (Backend, Frontend, Designer). Scope fleksibel — prioritas dan arah pengerjaan dapat disesuaikan setiap bulan. Pembayaran dilakukan setiap awal bulan di muka dengan kontrak minimum 3 bulan. Total biaya bergantung pada durasi aktual pengerjaan.

| Role | Per Bulan |
|------|-----------|
| Backend Developer | Rp 12.000.000 |
| Frontend Developer | Rp 12.000.000 |
| UI/UX Designer | Rp 8.000.000 |
| **Total/Bulan** | **Rp 32.000.000** |

---

## Perbandingan

| | Beli Putus (MVP) | Beli Putus (Post-MVP) | Retainer |
|--|-----------------|----------------------|---------|
| Harga/bulan | Rp 26.000.000 | Rp 26.000.000 | Rp 32.000.000 |
| Total | Rp 390.000.000 | Rp 52.000.000 | Tergantung durasi aktual |
| Durasi | 15–18 bulan | ~2 bulan | Tergantung durasi aktual |
| Scope | Phase 1–5 | Fitur Post-MVP | Fleksibel, dapat berubah |
| Fleksibilitas revisi | Terbatas | Terbatas | Lebih bebas |